...
function loadHighScore(gameMode) {
  ...
  document.getElementById("least-moves").textContent =
    modeScore.leastMoves === -1 ? "N/A" : modeScore.leastMoves;
  document.getElementById("least-time").textContent =
    modeScore.leastTime === -1 ? "N/A" : formatTimeShort(modeScore.leastTime);
}
...
function updateHighScore(gameMode, moves, time) {
  ...
  if (
    currentModeScore.leastMoves === -1 ||
    moves < currentModeScore.leastMoves
  ) {
                    ...
                    <div id="completed-overlay">
                        ...
                        <div style="display: flex; justify-content: space-between; width: 80%; margin: 20px auto; font-size:18px;">
                            <div style="text-align: left;">
                                <div>Moves: ${moves}</div>
                                <div>Least Moves: ${
                                  document.getElementById("least-moves")
                                    .textContent
                                }</div>
                            </div>
                            <div style="text-align: left;">
                                <div>Time: ${formatTimeShort(finalTime)}</div>
                                <div>Least Time: ${
                                  document.getElementById("least-time")
                                    .textContent
                                }</div>
                            </div>
                        </div>
